  ...
  <div>
    ...
    <div v-if="pathKey == entityName">
      <search-banner
        ...
        :span="permission.certificate_add || isDevProfile ? 21 : 24"
      ...
      >
        <template slot="menu">
          <el-button
            v-if="permission.certificate_add || isDevProfile"
            icon="el-icon-plus"
            size="small"
            @click="handleGrade({})"
            type="primary"
      ...
      >
        ...
        <template slot-scope="scope" slot="menu">
          <el-button
            v-if="permission.certificate_update || isDevProfile"
            icon="el-icon-edit"
            size="small"
          ...
          >
          <el-button
            v-if="permission.certificate_delete || isDevProfile"
            icon="el-icon-delete"
            size="small"
...
import upholdCaCertificate from "./uphold-cacertificate";
import searchBanner from "@/components/searchBanner";
import { findAllEntities, entityRemove } from "@/api/gateway/ca_certificates";
import ItemTags from "@/components/ItemTags";
import InnerBreadcrumb from "@/components/InnerBreadcrumb";
import CopyItem from "@/components/CopyItem";
export default {
  name: DIC.CACERTIFICATES + "_list",
  components: {
    ItemTags,
    searchBanner,
    upholdCaCertificate,
  ...
  data() {
    return {
      tableSearch: {},
      tableOption: [], //表格设置属性
      searchProps: [{ name: "tags", type: "array" }],
      tableData: [], //表格的数据
      tablePage: 1,
      tableSize: 10,
      tableLoading: false,
      ...
      pathKey: DIC.CACERTIFICATES,
      labelArgs: {},
      path: {
        label: DIC.CACERTIFICATES,
        key: DIC.CACERTIFICATES,
        children: [
          {
            key: DIC.CACERTIFICATES + "_add",
            label: "新建" + DIC.CACERTIFICATES,
          },
          {
            key: DIC.CACERTIFICATES + "_edit",
            label: "修改" + DIC.CACERTIFICATES,
          },
          {
            key: DIC.CACERTIFICATES + "_view",
            label: "查看" + DIC.CACERTIFICATES,
          },
        ],
      },
    };
